...
var _ = Describe("CyborgAPI controller", func() {
	...
	When("CyborgAPI is created with default config", func() {
		...
		It("configures HTTP probes on the API container and no probes on the log sidecar", func() {
			Eventually(func(g Gomega) {
				ss := th.GetStatefulSet(apiNames.StatefulSetName)

				// Log sidecar (index 0) should have no probes
				logContainer := ss.Spec.Template.Spec.Containers[0]
				g.Expect(logContainer.StartupProbe).To(BeNil())
				g.Expect(logContainer.ReadinessProbe).To(BeNil())
				g.Expect(logContainer.LivenessProbe).To(BeNil())

				// Main API container (index 1) should have HTTPGet probes
				container := ss.Spec.Template.Spec.Containers[1]
				...
				g.Expect(container.ReadinessProbe.HTTPGet.Port.IntValue()).To(Equal(6666))
				...
				g.Expect(container.LivenessProbe.HTTPGet.Port.IntValue()).To(Equal(6666))
	...
	When("CyborgAPI is created with custom probe overrides", func() {
		BeforeEach(func() {
			DeferCleanup(
				k8sClient.Delete, ctx,
				CreateCyborgAPIConfigSecret(apiNames.ConfigSecretName),
			...
			DeferCleanup(th.DeleteInstance, CreateCyborgAPICR(
				...
				cyborgv1beta1.CyborgAPISpec{
					CyborgAPITemplate: cyborgv1beta1.CyborgAPITemplate{
						Replicas: ptr.To(int32(1)),
						Override: cyborgv1beta1.APIOverrideSpec{
							Probes: probes.OverrideSpec{
								LivenessProbes:  &probes.ProbeConf{PeriodSeconds: 99, FailureThreshold: 7},
								ReadinessProbes: &probes.ProbeConf{PeriodSeconds: 55, FailureThreshold: 3},
							},
						},
					},
					ConfigSecret:   apiNames.ConfigSecretName.Name,
		...
		It("applies the custom probe parameters to liveness and readiness probes", func() {
			Eventually(func(g Gomega) {
				ss := th.GetStatefulSet(apiNames.StatefulSetName)
				container := ss.Spec.Template.Spec.Containers[1]
				g.Expect(container.LivenessProbe).NotTo(BeNil())
				g.Expect(container.LivenessProbe.PeriodSeconds).To(Equal(int32(99)))
				g.Expect(container.LivenessProbe.FailureThreshold).To(Equal(int32(7)))
				g.Expect(container.ReadinessProbe).NotTo(BeNil())
				g.Expect(container.ReadinessProbe.PeriodSeconds).To(Equal(int32(55)))
	...
	When("the Topology is updated after CyborgAPI is deployed", func() {
		It("updates the StatefulSet TopologySpreadConstraints", func() {
			DeferCleanup(
				k8sClient.Delete, ctx,
				CreateCyborgAPIConfigSecret(apiNames.ConfigSecretName),
			)

			topologyObj := CreateCyborgTopology(apiNames.APIName.Namespace, "api-topo-update")
			DeferCleanup(k8sClient.Delete, ctx, topologyObj)
			...
			// Verify the initial MaxSkew is 1
			Eventually(func(g Gomega) {
				ss := th.GetStatefulSet(apiNames.StatefulSetName)
				g.Expect(ss.Spec.Template.Spec.TopologySpreadConstraints).To(HaveLen(1))
				g.Expect(ss.Spec.Template.Spec.TopologySpreadConstraints[0].MaxSkew).To(Equal(int32(1)))
			}, timeout, interval).Should(Succeed())

			// Update the Topology's MaxSkew from 1 to 2
			Eventually(func(g Gomega) {
				...
				(*topology.Spec.TopologySpreadConstraints)[0].MaxSkew = 2
				g.Expect(k8sClient.Update(ctx, topology)).To(Succeed())
			}, timeout, interval).Should(Succeed())

			// Verify the StatefulSet gets updated with MaxSkew=2
			Eventually(func(g Gomega) {
				ss := th.GetStatefulSet(apiNames.StatefulSetName)
				g.Expect(ss.Spec.Template.Spec.TopologySpreadConstraints).To(HaveLen(1))
				g.Expect(ss.Spec.Template.Spec.TopologySpreadConstraints[0].MaxSkew).To(Equal(int32(2)))
			}, timeout, interval).Should(Succeed())
		})
	})
	...
	When("CyborgAPI is created with valid TLS certs", func() {
		...
		It("mounts CA bundle and per-endpoint TLS cert volumes in the StatefulSet", func() {
			Eventually(func(g Gomega) {
				ss := th.GetStatefulSet(apiNames.StatefulSetName)
				...
				g.Expect(secretNames).To(HaveKey(apiNames.CaBundleSecretName.Name))
				g.Expect(secretNames).To(HaveKey(apiNames.InternalCertSecretName.Name))
				g.Expect(secretNames).To(HaveKey(apiNames.PublicCertSecretName.Name))
			}, timeout, interval).Should(Succeed())
		})

		It("configures HTTPS probes when TLS is enabled", func() {
			Eventually(func(g Gomega) {
				ss := th.GetStatefulSet(apiNames.StatefulSetName)
				container := ss.Spec.Template.Spec.Containers[1]
				g.Expect(container.ReadinessProbe).NotTo(BeNil())
