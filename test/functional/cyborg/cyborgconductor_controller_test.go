...
var _ = Describe("CyborgConductor controller", func() {
	...
	When("CyborgConductor is created with default config", func() {
		...
		It("has exec-based startup and liveness probes and no readiness probe", func() {
			Eventually(func(g Gomega) {
				ss := th.GetStatefulSet(conductorNames.StatefulSetName)
				g.Expect(ss.Spec.Template.Spec.Containers).To(HaveLen(1))
				container := ss.Spec.Template.Spec.Containers[0]
				g.Expect(container.StartupProbe).NotTo(BeNil())
				g.Expect(container.StartupProbe.Exec).NotTo(BeNil())
				g.Expect(container.StartupProbe.Exec.Command).To(
					Equal([]string{"/usr/bin/pgrep", "-f", "-r", "DRST", "cyborg-conductor"}))
				g.Expect(container.LivenessProbe).NotTo(BeNil())
				g.Expect(container.LivenessProbe.Exec).NotTo(BeNil())
				g.Expect(container.LivenessProbe.Exec.Command).To(
					Equal([]string{"/usr/bin/pgrep", "-f", "-r", "DRST", "cyborg-conductor"}))
	...
	When("the Topology is updated after CyborgConductor is deployed", func() {
		It("updates the StatefulSet TopologySpreadConstraints", func() {
			...
			topologyObj := CreateCyborgTopology(conductorNames.ConductorName.Namespace, "conductor-topo-update")
			DeferCleanup(k8sClient.Delete, ctx, topologyObj)
	...
	When("CyborgConductor is created with custom probe overrides", func() {
		BeforeEach(func() {
			DeferCleanup(
				k8sClient.Delete, ctx,
				CreateCyborgConductorConfigSecret(conductorNames.ConfigSecretName),
			...
			DeferCleanup(th.DeleteInstance, CreateCyborgConductorCR(
				...
				cyborgv1beta1.CyborgConductorSpec{
					CyborgConductorTemplate: cyborgv1beta1.CyborgConductorTemplate{
						Replicas: ptr.To(int32(1)),
						Override: cyborgv1beta1.ConductorOverrideSpec{
		...
		It("applies the custom probe values to the StatefulSet", func() {
			Eventually(func(g Gomega) {
				ss := th.GetStatefulSet(conductorNames.StatefulSetName)
				container := ss.Spec.Template.Spec.Containers[0]
				...
				g.Expect(container.StartupProbe).NotTo(BeNil())
				g.Expect(container.StartupProbe.PeriodSeconds).To(Equal(int32(33)))
				g.Expect(container.StartupProbe.FailureThreshold).To(Equal(int32(15)))
